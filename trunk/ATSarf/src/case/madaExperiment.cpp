...
int mada(QString folderName, ATMProgressIFC * /*prg*/) {
			...
			if (analysis==correct_analysis || i==count) {
				if (tokenization==correct_tokenization)
					correct++;
                                        ...
                                        theSarf->out<<word<<"\t"<<correct_tokenization<<"\t"<<tokenization;
					if (equal(tokenization,correct_tokenization)) {
						...
						if (i==count) {
							not_found++;
                                                        theSarf->out<<"!!!";
                                                        theSarf->displayed_error<<word<<"\t"<<correct_analysis<<"\t"<<all_count<<"\n";
						} else {
							diff_incorrect++;
                                                        theSarf->out<<"!";
						}
	...
	input_tokenize.close();
        ...
        theSarf->displayed_error<<"Correct:\t\t"<<correct<<"/"<<all_count<<"=\t"<<((double)correct)/all_count<<"\n";
        theSarf->displayed_error<<"-Correct:\t\t"<<non_diacritics_correct<<"/"<<all_count<<"=\t"<<((double)non_diacritics_correct)/all_count<<"\n";
        theSarf->displayed_error<<"~Correct:\t\t"<<half_correct<<"/"<<all_count<<"=\t"<<((double)half_correct)/all_count<<"\n";
        theSarf->displayed_error<<"*Correct:\t\t"<<normalization_correct<<"/"<<all_count<<"=\t"<<((double)normalization_correct)/all_count<<"\n";
        theSarf->displayed_error<<">Incorrect:\t\t"<<larger_incorrect<<"/"<<all_count<<"=\t"<<((double)larger_incorrect)/all_count<<"\n";
        theSarf->displayed_error<<"<Incorrect:\t\t"<<smaller_incorrect<<"/"<<all_count<<"=\t"<<((double)smaller_incorrect)/all_count<<"\n";
        theSarf->displayed_error<<"!Incorrect:\t\t"<<diff_incorrect<<"/"<<all_count<<"=\t"<<((double)diff_incorrect)/all_count<<"\n";
        theSarf->displayed_error<<"Not Found:\t\t"<<not_found<<"/"<<all_count<<"=\t"<<((double)not_found)/all_count<<"\n";
	return 0;
}
