...
an Arabic morphological analyzer, which
%Sarf
allows an application-specific controller to control and refine
...
which greatly increases its ambiguity.
...
Moreover, determining word boundaries is not always 
%a trivial problem.
trivial.
For example,
\noTrnoVocRL{il_A\nospace almdrsT}
...
%ends with
%\noTrnoVocRL{_A} a non-connecting letter. 
The reason is that
\noTrnoVocRL{_A} is a non-connecting letter. 
%These words, referred to as 
Such 
...
The controller can be a Finite State Automaton (FSA) or any other %logical entity 
...
Moreover,
% since Sarf does not perform white-space delimited tokenization, 
Sarf 
...
%finite state machine (FSM) based analyzers as described in~\cite{Beesley:03}.
...
Sarf, however, allows users to write directly the code for the needed controller. %(maybe not clear)
...
Similar to Sarf, both Beesley %~\shortcite{Beesley:01} (already cited previously)
and Attia~\shortcite{Attia:05} use
...
ElixirFM & 166.8$\times60$ & 138$\times60$ & 29.2$\times$60 \\
\hline
\end{tabular}
}
\normalsize
