...
void AMTMainWindow::createDockWindows(bool open) {
    ...
    connect(tagDescription, SIGNAL(itemClicked(QTreeWidgetItem *, int)), this, SLOT(itemSelectionChanged(QTreeWidgetItem *,
            int)));
...
void AMTMainWindow::tagWord(int start, int length, QColor fcolor, QColor  bcolor, int font, bool underline, bool italic,
                            bool bold) {
    QTextBrowser *taggedBox = txtBrwsr;
    QTextCursor c = taggedBox->textCursor();
    ...
    for (int j = 0; j < _atagger->msfVector->count(); j++) {
        if ((_atagger->msfVector->at(j))->name == type) {
            ...
            for (int k = 0; k < merftag->relationMatchVector.count(); k++) {
                ...
                Triplet<Agnode_t *, Agnode_t *, QString> *triplet = new Triplet<Agnode_t *, Agnode_t *, QString>(node1, node2,
                        relMatch->edgeLabel);
...
void AMTMainWindow::difference() {
    ...
    foreach (QVariant type, resulttt["TagTypeSet"].toList()) {
        ...
        if (!typeElements.value("Features").isNull()) {
            ...
            _atagger->isSarf = true;
            _atagger->insertSarfTagType(tag, tags, desc, foreground_color, background_color, font, underline, bold, italic, sarf,
                                        compareTo);
        } else {
            _atagger->isSarf = false;
