...
CustomSTTView::CustomSTTView(QWidget *parent) :
    ...
    connect(listSelectedTags, SIGNAL(itemClicked(QTreeWidgetItem *, int)), this,
            SLOT(selectedTags_clicked(QTreeWidgetItem *, int)));
    ...
    for (int i = 0; i < _atagger->tagTypeVector->count(); i++) {
        ...
        if (_atagger->tagTypeVector->at(i)->source == sarf) {
            ...
            QVector< Quadruple< QString , QString , QString , QString > > tags =
                QVector< Quadruple< QString , QString , QString , QString > >(_stt->tags);
...
void CustomSTTView::btnAdd_clicked() {
    bool ok;
    QString tagName = QInputDialog::getText(this, "Tag Name", "Please insert a Tag Type Name", QLineEdit::Normal, QString(),
                                            &ok);
    ...
    SarfTagType *sarftagtype = new SarfTagType(tagName, tags, QString(), fgcolor, bgcolor, font, underline, bold, italic,
                                               sarf);
...
void CustomSTTView::btnLoad_clicked() {
    ...
    if (fileName.isEmpty()) {
        ...
        QJsonDocument jsonDoc = QJsonDocument::fromJson(sarfTT);
        ...
        foreach (QVariant type, result["TagTypeSet"].toList()) {
            ...
            SarfTagType *stt = new SarfTagType(tag, tags, desc, foreground_color, background_color, font, underline, bold, italic,
                                               sarf);
            sttVector->append(stt);
        }
