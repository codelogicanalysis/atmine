...
QString getColumn(QString table, QString column_name, long long id, QString additional_condition = "",
                  bool has_id = true);
...
dbitvec addSource(QString table, int source_id, long long id = -1 , QString additional_condition = "",
                  bool has_id = true);
...
long insert_category(QString name, item_types type, dbitvec sources,
                     bool isAbstract = false); //returns its id if already present and if names are equal but others are not, -1 is returned
long insert_category(QString name, item_types type, int source_id,
                     bool isAbstract = false); //returns its id if already present
...
long insert_item(item_types type, QString name, QString raw_data, QString category, int source_id,
                 QList<long> *abstract_ids, QString description = "", QString POS = "", QString grammar_stem = "",
                 QString lemma_ID = "");
