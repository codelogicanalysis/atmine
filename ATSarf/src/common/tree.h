...
class tree {
    ...
    protected:
        ...
        node *addElement(QString letters, long affix_id, long category_id, long resulting_category_id, bool isAccept,
...
inline bool isAcceptState(item_types type, long cat_r_id) {
    ...
    if (type == PREFIX) {
        Retrieve_Template existABcheck("compatibility_rules", "COUNT(*)",
                                       QString("category_id1=%1 AND type=%2").arg(cat_r_id).arg((int)AB));

        if (existABcheck.retrieve() && existABcheck.get(0).toInt() > 0) {
            Retrieve_Template existACcheck("compatibility_rules", "COUNT(*)",
                                           QString("category_id1=%1 AND type=%2").arg(cat_r_id).arg((int)AC));
        ...
        Retrieve_Template existACcheck("compatibility_rules", "COUNT(*)",
                                       QString("category_id2=%1 AND type=%2").arg(cat_r_id).arg((int)AC));

        if (existACcheck.retrieve() && existACcheck.get(0).toInt() > 0) {
            Retrieve_Template existBCcheck("compatibility_rules", "COUNT(*)",
                                           QString("category_id2=%1 AND type=%2").arg(cat_r_id).arg((int)BC));
