...
class ColorIndices {
    private:
        ...
        void setGraph(NarratorGraph *graph) {
            this->graph = graph;
        }
...
class GraphVisitorController {
    ...
    private:
        ...
        void init();
        void construct(NodeVisitor *visitor, NarratorGraph *graph, bool keep_track_of_edges, bool keep_track_of_nodes,
    ...
    public:
        ...
        void finish();
        unsigned int getVisitColorIndex() {
            return visitIndex;
        }
        unsigned int getFinishColorIndex() {
            return finishIndex;
        }
        const ParentStack &getParentStack() {
            return parentStack;
        }
        NarratorGraph *getGraph() {
            return graph;
        }
        bool isTop2Bottom() {
            return direction;
...
#define d_out (*dot_out)
        ...
        virtual QString getOtherAttributes(NarratorNodeIfc &) {
            return "";
        }
...
class LoopBreakingVisitor: public NodeVisitor {
    ...
    public:
        LoopBreakingVisitor(double equality_threshold) {
...
class NarratorGraph {
    public:
        class HadithFileDetails {
            public:
                HadithFileDetails(QString fileName) {
                    this->fileName = fileName;
                }
                HadithFileDetails(QDataStream &in) {
                    deserialize(in);
                }
                QString fileName;
    ...
    private:
        ...
        class BuildAction: public NarratorHash::FoundAction {
            private:
                ...
                bool merged;
            private:
                ChainNarratorNode *cn() {
                    return dynamic_cast<ChainNarratorNode *>(node);
                }
                GroupNode *gn() {
                    ...
                    if (
                        ...
                        if (merged) {
                            ...
                            if (group == NULL) {
                                ...
                                graph->mergeNodes(*hashedNode, *group,
                                                  toDelete); //since both are not null, the one to be deleted is the second which is already not in the hash not the first
                            }
                        }
                    }
                }
        ...
        NodeIterator findEquivalent(ChainNarratorNode *c, NarratorNodeIfc *n) {
            ...
            for (; !itr.isFinished(); ++itr) {
                ...
                if (c->getNarrator().getStart() == c2->getNarrator().getStart() &&
                    c->getNarrator().getEnd() == c2->getNarrator().getEnd()) {
                    return itr;
                }
            }
        ...
        void checkAllGroupHaveCorresponding() {
            // Useless method. References cannot be bound to dereferenced NULL pointer
            //for (int i=0;i<all_nodes.size();i++){
        ...
        class DFS_Helper {
            ...
            public:
                ...
                DFS_Helper(GraphVisitorController &aVisitor, NarratorNodeIfc *node, int direction = 1,
                           int levels = -1): visitor(aVisitor), maxLevels(levels) {
    ...
    public:
        ...
        static void BFS_traverse(GraphVisitorController &visitor, int maxLevels, NarratorNodeIfc *node = NULL,
                                 int direction = 1) {
            //if direction>0, we go to child,else to parent
