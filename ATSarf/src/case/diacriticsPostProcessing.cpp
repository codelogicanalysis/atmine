...
QString DiacriticsPostProcessing::displayFeature(int , QString feature) {
	return feature;
}
...
void DiacriticsPostProcessing::restartLumpedEntries(QString newUnique, QString newFeature) {
	if (!lastUnique.isEmpty()) {
		double currAmb=lastAverage.evaluate();
		StatTable::iterator itr=stats.find(lastFeature);
		if (itr==stats.end()) {
			Average a(currAmb);
			stats.insert(lastFeature,a);
		} else {
			Average & av=*itr;
			av.add(currAmb);
		}
...
void DiacriticsPostProcessing::processLine(QStringList list) {
	...
	if (uniqueKey==lastUnique) { //will be lumped with old
		lastAverage.add(currAmb);
	} else {
		QString featureKey=getKey(list,testedFeatures);
		restartLumpedEntries(uniqueKey,featureKey);
		lastAverage.add(currAmb);
	}
}
...
OneDiacriticEvaluation::OneDiacriticEvaluation(IndexList features,Ambiguity am) {
	...
	testedFeatures.append(features);
