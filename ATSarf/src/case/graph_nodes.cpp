...
int ChainNarratorNode::getSavedRank() const {
    return (&((ChainNarratorNode *)this)->getCorrespondingNarratorNode() ==
            this //to make sure it is not the case of an empty graph node even if it belongs to a group node
            ? savedRank
            : group->getSavedRank());
}
