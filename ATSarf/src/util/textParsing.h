...
class PunctuationInfo {
    public:
        ...
        bool hasEndingPunctuation() {
            return fullstop || newLine;
        }
        bool hasParagraphPunctuation() {
            return fullstop && newLine;
        }
        bool update(const QChar &letter) {  //returns true if this letter is a delimiter
            if (non_punctuation_delimiters.contains(letter)) {
                ...
                if (letter == ',' || letter == fasila) {
                    ...
                    semicolon = true;
                } else if (letter == full_stop1 || letter == full_stop2 || letter == full_stop3 || letter == question_mark ||
                           letter == '.' || letter == '?') {
                    fullstop = true;
