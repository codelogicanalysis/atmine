...
/**
  ...
  */
bool checkIfFirstIsContainedInSecond(const QStringRef &word1, const QStringRef &word2, int &i1, int &i2,
                                     bool force_shadde = false); //modifies value of i1 and i2
...
/**
  ...
  */
bool checkIfSmallestIsPrefixOfLargest(const QStringRef &word1, const QStringRef &word2, int &i1, int &i2,
                                      bool force_shadde = false); //modifies value of i1 and i2
...
/**
  ...
  */
inline bool equal(const QStringRef &word1, const QStringRef &word2, bool force_shadde = false,
                  bool force_last = true) { // is diacritics tolerant and ignores punctuation
    int i1, i2;
...
inline bool equal(const QString &word1, const QString &word2, bool force_shadde = false,
                  bool force_last = true) { // is diacritics tolerant
    return equal(word1.rightRef(-1), word2.rightRef(-1), force_shadde, force_last); //rightRef of <0 returns whole string
}

inline bool equal(const QStringRef &word1, const QString &word2, bool force_shadde = false,
                  bool force_last = true) { // is diacritics tolerant
...
/**
  ...
  */
inline bool startsWith(const QStringRef &text, const QString &substring,
                       int &finish_pos) { // is diacritics tolerant checks if a word ends with a delimeter terminated 'subset'
...
inline bool startsWith(const QString &text,
                       const QString &substring) { // is diacritics tolerant checks if a word ends with a delimeter terminated 'subset'
    int dummy_int;
...
/**
  ...
  */
inline bool startsWithAL(const QString
                         &word) { //does not take in account cases were Diacritics may be present on the alef and lam of "al"
    long i = 0;
    ...
    */
    ...
    for (int i = 0; i < st1.size(); i++) {
        for (int j = 0; j < st2.size(); j++) {
            count += commonWords(text, st1[i],
                                 st2[j]); //if has no common characters will return zero, so although not efficient is expected to return sane results
        }
    }
