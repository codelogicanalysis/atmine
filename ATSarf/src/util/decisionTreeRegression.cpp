...
void DecisionTreeRegression::initialize(QString fileName, QList<int> &featureColumns, int targetColumn,
                                        int weightColumn, ATMProgressIFC *prg) {
    QString tempFileName = fileName + ".temp";
    QFile temp(tempFileName);
    temp.remove();

    if (!temp.open(QIODevice::WriteOnly)) {
        _error << "Unable to open temporary file '" << tempFileName << "'\n";
        return;
    }
    ...
    if (!file.open(QIODevice::ReadOnly)) {
        _error << "Unable to open file '" << fileName << "'\n";
        return;
    }
...
DecisionTreeRegression::~DecisionTreeRegression() {
    if (!QFile::remove(fileName)) {
        _error << "Unable to delete temporary file '" << fileName << "'\n";
    }
}

DecisionTreeRegression::DecisionTreeRegression(QString fileName, QList<int> featureColumns, int targetColumn,
                                               int weightColumn, ATMProgressIFC *prg) {
    initialize(fileName, featureColumns, weightColumn, targetColumn, prg);
}

DecisionTreeRegression::DecisionTreeRegression(QString fileName, QList<int> featureColumns, int targetColumn,
                                               int weightColumn, TerminationRule rule, ATMProgressIFC *prg) {
    initialize(fileName, featureColumns, weightColumn, targetColumn, prg);
    this->terminationRule = rule;
}


RegressionNode *DecisionTreeRegression::buildTreeNode(QList<int> workingfeatureColumns, ConditionMap map,
        ATMProgressIFC *prg) {
    if (workingfeatureColumns.size() > 0) {
        ...
        if (bestValues.size() > 0) {
            ...
            for (int i = 0; i < bestValues.size(); i++) {
                ...
                if (!(stDev < terminationDevThreshold || workingfeatureColumns.size() == 0 ||
                      numInstances < terminationRule.minInstances)) {
