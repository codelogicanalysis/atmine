...
class Diacritics {
    ...
    public:
        ...
        bool hasShadde() const {
            assert(!inconsistent);
            return shadde;
        }
        Diacritic getMainDiacritic() const {
            assert(!inconsistent);
            return main;
        }
        ...
        bool isEmpty() const {
            return !hasShadde() && main == UNDEFINED_DIACRITICS && isSelfConsistent();
        }
};

inline bool isDiacritic(const QChar &letter) {
    if (letter == shadde || letter == fatha || letter == damma || letter == kasra ||
...
inline QString removeDiacritics(QString /*&*/text) {
    QRegExp exp(QString("[") + shadde + fatha + damma + sukun + kasra + kasratayn + fathatayn + dammatayn +
                aleft_superscript + QString("]"));
    return text.remove(exp);
}
inline int getLastLetter_index(const QString &word,
                               int pos) { //last non-Diacritical letter, -1 means that all letters are diacritics
...
inline QString removeLastDiacritics(const QString
                                    &word) { //removes last consecutive diactrics until a normal letter is reached
...
inline int getLastDiacritic(int position,
                            QString *diacritic_word) {  //get letter position after last diacritic starting from 'position+1', if no diacritic is found returns 'position+1'
