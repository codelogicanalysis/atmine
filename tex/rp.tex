...
Completion of the above goals would result in a significant contribution to the research community of NLP in general and Arabic Analysis in particular. However, as already noted, text mining is a wide problem that cannot be expected to saturate in the near future especially when it comes to the arabic language where much work is lacking. Consequently, we look forward for additional contributions to the field such as the following:
\begin{itemize}
\item Enhance morphological analysis: It is always the case, that the lexicon upon which the morphological analyzer can be augmented with useful categorization and entity tags that reflect semantical meanings. These modifications would give CATARM additional capabilities in terms of allowing users to extract and use a greater variety of entities when building instructive queries. The feedback of experts in the field of arabic linguistics can continually add accuracy to the morphological solutions.  
\item Look for other optimizations: From our priliminery research and results we were able to envision some optimizations to current tools; that's why it is expected that as we develop the complete framework to stumble at much more ideas and suggestions for improving the text mining process we are conducting expecially in terms of efficency and accuracy.
\item Generalize the framework to other Arabic-like languages: Some sematic languages share many common features to the Arabic language such as the Amharic, Hebrew, etc... or even are written using the Arabic script like Persian, Kurdish, Urdu, Pashto. Accordingly, one may consider porting our techniques to them, or even generalizing the framework to support for languages other than Arabic.
\item Generalize the case-based approach: ??
\item Integrate other text mining techniques such as vocalization. Vocalization refers to the process of adding diacritics to an unvocalized text. The prerequisites of a successful vocalizer are the accuracy of the morphological analyzer in addition to grammatical analyzers. Such tools are especially useful in the academic sector where fully vocalized texts may be of high utility. 
\item Maintenance: We should never forget that for any software to remain useful, it must be continuously monitored to correct bugs, improve its interface, and enrich its capabilities. That's why CATARM should undergo extensive testing, fixes, and enhancements even after launching. 
\end{itemize}
