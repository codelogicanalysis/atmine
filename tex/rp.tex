...
Our LCM will differ from ElixirFM~\cite{Otakar:07}
in that it will follow a relational approach.
In a relation, an element can have several associates.
In a function, an element is mapped to only one associate. 
We will express our morphological solutions in 
a directed acyclic graph (DAG)
representing a hierarchy of relations between the morphemes.
This is more succinct than morphological trees mapping the 
morphemes to the solutions as in~\cite{Otakar:07}.
Furthermore, interesting relations between the morphemes 
can be computed statically before
the input is presented to the analyzer.

In~\cite{Dao09}, Daoud observes that the pipeline model,
where morphology is performed first and syntactic and other
processes follow, is not adequate for Arabic. 
He suggests a synchronized syntactic and morphological
approach.  
We take the argument further to 
allow a tighter integration between the several pipeline
levels up to the NLP query in question by providing a 
case-based morphological analyzer, a linguistic computational
model of Arabic, and a query language that can all be expressed
as FSMs. 
The work~\cite{AlSham08} reports a similar result where
the automated addition of syntax knowledge improved stemming
in terms of accuracy and efficiency. 

Here we point to a subtlety that shows how complex 
low level Arabic morphology is. 
The work in~\cite{Dao09} assumes that a noun
cannot be followed by a preposition in Arabic and uses that 
to resolve an ambiguity in the 
example~\noTrNoVocRL{.s-a.hb-aa s-amy _dhb-aa il_A alswq} 
(Sami's two companions went to the market) 
    and decides that \noTrNoVocRL{_dhb-aa} 
is a verb (they went)
and not a noun since it is followed by \noTrNoVocRL{il_A}.
One can 
say~\noTrNoVocRL{a_h_d  .s-a.hb-aa s-amy _dhb-aa il_A alswq} 
(Sami's two companions took gold to the market) and 
now \noTrNoVocRL{_dhb-aa} is a noun (gold stressed with double 
diactric).
This shows that even complex and advanced techniques such as
~\cite{Dao09} can be misled by ambiguous compatibility
rules. 
