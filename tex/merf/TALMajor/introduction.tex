...
We refer to annotations and labels as {\em tags} and {\em tag types}, respectively, in the sequel.
...
prefix, stem, suffix, part of speech (POS) tags, gloss tags, extended synonym tags, and semantic categories.
We often refer to regular expressions and Boolean formulae as expressions and formulae, respectively, for short.
...
zero or one, zero or more, one or more, up to $M$ \major{repititions where $M$ is 
...
\framework takes an Arabic text, a set of user-defined Boolean 
formulae, and a set of user-defined regular expressions.
...
This work significantly extends the position paper~\cite{JaZaMatar} that allows for manual, and basic morphology annotation.
...
\begin{itemize}
  ...
  \item \framework enables the user to tag words based on a 
    \major{
    novel ligh Arabic wordnet relation that leverages the 
    synonym $Syn^k$ feature.
  }

  \item \major{
\framework is open source and available online for the research community 
