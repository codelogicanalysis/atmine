...
\done{
  We combine the Motivation section with \framework section (Now Section~\ref{sec:framework}).
}
...
\answer{
F is the set of morphological features where each element is in turn a set of the values of that feature.
Hence, A is one such set corresponding to a morphological feature, and CF is a feature value in A.
We clarify this point in Subsection~\ref{subsec:grammar}.
}

\done{
  DONE: Ameen please find where you fixed this and say it here. 
}
...
\done{
  ...
  Added the above explanation in the Results section, needs to be checked.
}
...
\done{
  highlight the changes and refer to them please.\\
  We use R first to denote the set of user-defined relations.
  Then, we use it to denote the set of MBF tags which is contradicting.
  I got rid of the first case as we don't need it.
}
