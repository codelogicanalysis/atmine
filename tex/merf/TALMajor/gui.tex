...
The user writes MBF tag types with the tag type editor introduced in~\citename{JaZaMatar}~\citeyear{JaZaMatar}. %shown in Figure~\ref{f:bfe}.
...
\begin{figure}[tb]
  ...
  \fbox{\includegraphics[width=0.995\textwidth]{figures/mreedit}}}
  \vspace{-2em}
...
%then we apply the one or more ($+$) operation to the resulting subexpression.
...
%RE matches In order to add computational actions to an MRE subexpression, 
...
%\framework provides the user with an API to access the subexpression match features easily and define the pre-match actions. 
