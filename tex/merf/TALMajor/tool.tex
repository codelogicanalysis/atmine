...
a feature containing or exactly matching \major{the user-chosen feature value} $CF$.
\item $a(w) := w \in Syn^k(CF), CF \in \ss$.
  Informally, this checks if $w$ is an extended synonym of a stem $CF$. 
\major{We limit $k$ to a maximum of $7$ since we practically noticed that 
...
\item $fg$ is an MRE 
  \major{
  where $f$ and $g$ are both MRE expressions. 
    A match of $f$ followed by a match of $g$ satisfies this concatenation operation.}
\item $f*,f+,f$\textasciicircum$x,$ and $f?$ \major{ are MRE } where $f$ is an MRE,
  and are satisfied by zero or more, \major{ one or }more, up to $x$ matches, 
    and an \major{optional single match} of $f$, respectively.
  \item $f\& g,$ (conjunction) and $f|g$ (disjunction) are \major{MRE} where 
    $f$ and $g$ are \major{MRE}, and are satisfied by 
    \major{the intersection of $f$ and $g$ matches, 
...
\major{
The \major{matching parse trees} in Figure~\ref{fig:taskMRE} illustrate two matches of the expression computed by \framework.
...
Similarly, the matches to $e_1$, $o_1$, $r$, $o_2$, and $e_2$ in the second \major{matching parse tree} are
...
Each sub-expression can be associated with two computational 
...
Once \framework computes all \major{matching parse trees}, it traverses each tree to 
...
A simulation match $m$ of an expression $f$ is a \major{parse tree} where the root spans the expression, the internal nodes are roots to sub-expressions of $f$, and the leaves are matches of the MBF formulae of $f$, e.g. Fig.~\ref{fig:taskMRE}.
