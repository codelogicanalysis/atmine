...
Similarly, the matches to $e_1$, $o_1$, $r$, $o_2$, and $e_2$ in the second matching parse tree are \RL{dby mwl} (Dubai Mall), \RL{`l_A} (is located), \RL{mqrbT} (near), \RL{mn h_dA} (this), and \RL{Almbn_A} (building), respectively.
...
supported in~\cite{sipser2012introduction}, by 
...
A simulation match $m$ of an expression $f$ is a parse tree where the root spans the expression, the internal nodes are roots to sub-expressions of $f$, and the leaves are matches of the MBF formulae of $f$, e.g. Fig.~\ref{fig:taskMRE}.
