...
Tables~\ref{t:hadithresults} and \ref{t:nopunctlearnresults}. We used recall and precision as our evaluation metrics.
...
in reporting the correct number of narrators in the sanad. Finally, the narrator boundary refers
to the accuracy in reporting the correct number of words inside each narrator.

Precisely defined, segmentation recall refers to the fraction of extracted narrations from 
those which are correct; and segmentation precision refers to the fraction of correct narrations
from those extracted. Similarly, sanad boundary recall is a measure whether we detected all narrators
in the sand; and precision is a measure whether we did so without introducing much false positives.

iTree does not tackle the problem hadith segmentation. In contrast to NaGEMA, it does not work on a book level, 
instead it assumes that the knowledge of what part of the text of a hadith book is a narration is available.
Consequently, for the narrators to be extracted, the hadith must be manually extracted
...
Now we move to explain the significance of the experiments performed.
