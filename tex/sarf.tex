...
Beesly~\shortcite{Beesley:01}, SAMA~\cite{Kulick:10},
...
Buckwalter~\shortcite{Buckwalter:02}  and use define the affixes
recursively. 
...
\caption{Example composition of the prefix, stem and suffix transducers.}
\label{f:example}
}
\end{figure*}
% the FSMs for the three words
...
prefix transducer corresponding to the suffix \RL{ws}. 
...
suffix transducer. When \RL{h} is consumed, $\Phi$ will die since 
...
Running time (secs.)& & 6.65 & 12$\times$60& &4.55 & 2.3$\times$3600 & & .66 & 29.2$\times$60 
\end{tabular}
\normalsize
