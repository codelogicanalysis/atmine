...
Sarf uses 
concatenative analysis with a novel 
{\em recursive} 
refinement. 
...
the controller and 
the prefix, stem, and suffix deterministic tansducers. 
...
To acheive that we ran Sarf in two modes, simple and refined.
...
We think the big difference is due to the sofistication of 
grammar based functional morphological analysis ElixirFM 
uses and its reliance on additional annotations extracted from both ATB~\cite{Maamouri:04} and the Pragbank~\cite{Prague04}.
...
explored partialy based on the case study. 
